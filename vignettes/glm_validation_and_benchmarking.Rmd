...
3. Implementation on Large Dataset and Computational Efficiency Benchmarking

We now firstly validate the optimized C-level glm() using pure R glm() coefficients as the baseline on a large dataset and then test the computational speed on the same dataset to compare the pure R implementation with the optimized C-level glm().
```{r efficiency_data}
...
summary(MyGLM_Optimized)
summary(BaseGLM)

# Compare coefficients

is_equal <- all.equal(
as.vector(MyGLM_Optimized$coefficients),
as.vector(coef(BaseGLM)),
tolerance = 1e-4
)
